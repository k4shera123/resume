...
const translations = {
  ru: {
    ...
    educationContent:
      "<h3>Полоцкий Государственный Университет</h3><p>Факультет Информационных Технологий<br />Специальность: Мобильные системы<br /><em>2024 – 2028</em></p>",
  ...
  en: {
    ...
    educationContent:
      "<h3>Polotsk State University</h3><p>Faculty of Information Technology<br />Specialty: Mobile Systems<br /><em>2024 – 2028</em></p>",
    ...
    disciplinesPageTitle: "STUDY DISCIPLINES",
  ...
  zh: {
    ...
    educationTitle: "教育背景",
    educationContent:
      "<h3>波洛茨克国立大学</h3><p>信息技术学院<br />专业：移动系统<br /><em>2024 – 2028</em></p>",
...
function updateContent(lang) {
  ...
  document.querySelector('.about .card').innerHTML = t.aboutMeParagraphs.map(p => `<p>${p}</p>`).join('');
  ...
  const whyMeUl = document.querySelector('.why-me ul');
  whyMeUl.innerHTML = '';
  t.whyMeList.forEach(item => {
    const li = document.createElement('li');
    li.textContent = item;
    whyMeUl.appendChild(li);
  });

  // Update disciplines page if exists
  const disciplinesHeader = document.querySelector('.disciplines-header h1');
  if (disciplinesHeader) {
    disciplinesHeader.textContent = t.disciplinesPageTitle;
    
    const backBtn = document.querySelector('.back-button');
    if (backBtn) {
      backBtn.innerHTML = `<i class="fas fa-arrow-left"></i> ${t.backButton}`;
    }
